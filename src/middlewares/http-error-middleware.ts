...
const HttpErrorMiddleware = async (
  error: MyMoviesException | Error | ExpressJoiError | any,
  request: Request,
  response: Response,
  next: NextFunction
) => {
  if (error instanceof MyMoviesException) {
    ...
    if (error.code === 'ENOTFOUND') {
      const newError = new MyMoviesException(
      ...
      const exception = new MyMoviesException(
        err,
        error.response.data.message,
        error.stack
      );
      ...
      const err = new MyMoviesException(
    ...
    let validationError: MyMoviesException;
    if (error.type === 'headers') {
      validationError = new MyMoviesException(
